# analyzers/statistics/market_stats_aggregator.py
...
class MarketStatsAggregator:
    ...
    def _aggregate_all_stats(self) -> Dict:
        ...
        if 'risk_management' in self.services:
            risk_service = self.services['risk_management']
            risk_status = risk_service.get_risk_status()
            aggregated['services']['risk'] = risk_status
        
        # Volume Profile Analyzer
        if 'volume_profile' in self.services:
            vp_service = self.services['volume_profile']
            for symbol in ['WDO', 'DOL']:
                profile = vp_service.get_profile(symbol)
                aggregated['services'][f'volume_profile_{symbol}'] = profile
        
        return aggregated
    
    def _update_aggregated_stats(self, current_data: Dict):
    ...
    def _update_market_profile(self, services: Dict):
    ...
    def _update_flow_metrics(self, services: Dict):
    ...
    def _update_time_series(self, services: Dict):
    ...
    def _update_signal_quality(self, services: Dict):
    ...
    def _handle_tape_reading(self, data: Dict):
        pass
    
    def _handle_arbitrage(self, data: Dict):
        pass
